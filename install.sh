...
if ! grep --quiet "the-crispy.docker" "$HOSTFILE"; then
	echo "Adding DNS to hosts file for the-crispy.docker."
    sudo cp "$HOSTFILE" "$HOSTFILE".bak
	sudo -- sh -c -e "echo '\n# Bacon is Crispy...\n192.168.99.100\tthe-crispy.docker www.the-crispy.docker' >> /etc/hosts"
	dscacheutil -flushcache
else
	echo "DNS information for tribbles.docker exists in hosts file. Continuing with installation."
fi
